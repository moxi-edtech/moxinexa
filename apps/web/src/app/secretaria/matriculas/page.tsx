...
export default async function Page(props: { searchParams?: Promise<SearchParams> }) {
  ...
  if (!escolaId) {
    return (
      <PortalLayout>
        <AuditPageView portal="secretaria" action="PAGE_VIEW" entity="matriculas_list" />
  ...
  if (q) {
    // filtra por status aproximado; se q parecer uuid, tente por id exato
        ...
        <div className="flex items-end justify-between gap-4 mb-4">
          <div>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2 text-xs">
              ...
              {['1','7','30','90'].map((d) => (
                <a key={d} href={`/secretaria/matriculas?days=${encodeURIComponent(d)}&q=${encodeURIComponent(q)}`} className={`px-2.5 py-1 rounded border ${days === d ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-700 border-gray-200 hover:bg-gray-100'}`}>{d === '1' ? '1 dia' : `${d} dias`}</a>
              ))}
          ...
          <form action="" className="flex gap-2 text-sm">
            <input type="text" name="q" placeholder="Buscar (status/UUID)" defaultValue={q} className="border rounded px-2 py-1" />
