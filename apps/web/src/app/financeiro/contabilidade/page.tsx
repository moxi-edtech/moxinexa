...
export default async function Page() {
  ...
  if (!escolaId) {
    return (
      <PortalLayout>
        <AuditPageView portal="financeiro" action="PAGE_VIEW" entity="contabilidade" />
        <div className="p-4 bg-amber-50 border border-amber-200 rounded text-amber-800 text-sm">
          Vincule seu perfil a uma escola para acessar integração contábil.
        </div>
      </PortalLayout>
    )
  }
      ...
      <div className="bg-white rounded-xl shadow border p-5">
        ...
        {!allowed ? (
          ...
          <div className="text-sm text-gray-600">
            Em breve: Exportações oficiais e integração com sistemas de contabilidade externos.
          </div>
        )}
      </div>
    </PortalLayout>
