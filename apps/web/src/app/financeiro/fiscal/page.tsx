...
export default async function Page() {
  ...
  if (!escolaId) {
    return (
      <PortalLayout>
        <AuditPageView portal="financeiro" action="PAGE_VIEW" entity="fiscal" />
        <div className="p-4 bg-amber-50 border border-amber-200 rounded text-amber-800 text-sm">
          Vincule seu perfil a uma escola para acessar o módulo fiscal.
        </div>
      </PortalLayout>
    )
  }
      ...
      <div className="bg-white rounded-xl shadow border p-5">
        ...
        {!allowed ? (
          ...
          <div className="text-sm text-gray-600">
            Em breve: Emissão de NF-e, integração AGT e relatórios fiscais.
          </div>
        )}
      </div>
    </PortalLayout>
